...
The source code builds upon [Simple-Evals](https://github.com/openai/simple-evals) by OpenAI. We focus on the MMLU-PRO benchmark and include various samplers and evaluation metrics explored in our paper.
...
If the model is not supported by vLLM, use the Hugging Face backend:
```bash
...
After sampling, initiate evaluation by running:
```bash
python -m LLM-Calibration-Study.simple_evals --model MODEL_NAME --benchmark mmlu_pro --conf_mode eval_all
```
Replace MODEL_NAME with one of the following supported models:

<details> <summary>Click to expand full model list</summary>
